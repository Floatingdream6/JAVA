...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top" style="z-index:106;">
    ...
    <ul class="navbar-nav">
        ...
        <li class="nav-item">
            <a class="nav-link" href="/ShopDemo_war_exploded/JspPage/AddNewShopPage.jsp">新增商品</a>
        </li>
    </ul>
</nav>
...
<table>
    ...
    <tr>
        ...
        <td>
            <button type="button" class="btn btn-primary " onclick="javascript:location.href='/ShopDemo_war_exploded/JspPage/ModifyShopInfoPage.jsp?shopname=<%= list.get(i).getShopname() %>'">修改</button>
            &nbsp;&nbsp;&nbsp;
            <button type="button" class="btn btn-danger" onclick="show_confirm('<%=list.get(i).getShopname()%>');">删除</button>
        </td>
    </tr>
    <%
        }
...
<script>
    ...
    function show_confirm(name)
    {
        var r=confirm("确认删除?");
        if (r==true) {
            location.href="/ShopDemo_war_exploded/Servlet/DeleteShopServlet?shopname="+name+"";
        }
        else {
        }
    }
